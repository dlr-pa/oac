...
def adapt_co2_emission(config, emis_interp_dict) -> tuple[np.ndarray, dict]:
    ...
    if enabled and "CO2" in emis_interp_dict:
        if mode == "direct":
            emis_interp_dict["CO2"] = emis_interp_dict["CO2"] * get_factor(
                config, "CO2"
            )
        elif mode == "incremental":
            n_steps = np.size(emis_interp_dict["CO2"])
            emis_interp_dict["CO2"] = emis_interp_dict["CO2"] * ratio_change(
                get_factor(config, "CO2"), n_steps, time_horizon
            )
        else:
            raise ValueError("Unkown mode")
    if enabled and "CO2" not in emis_interp_dict:
    ...
    for spec in species_list:
        if enabled:
            if mode == "direct":
                ...
                rf_interp_dict[spec] = rf_interp_dict[spec] * ratio_change(
                    get_factor(config, spec), n_steps, time_horizon
                )
            else:
                raise ValueError("Unkown mode")
