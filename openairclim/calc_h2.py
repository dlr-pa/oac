...
from read_netcdf import open_netcdf
...
class EmissionModel:
    ...
    def read_aviation_fuel_consumption(self):
        ...
        if self.cs is None:
            ...
            fn = glob.glob(self.wd + f'\\repository\\h2\\fuel_consumption_scenarios\\{self.cs}*')
...
class BoxModel:
    ...
    def get_perturbation(self, ds_eh2):
        ...
        ds['dch4'] = xr.DataArray((y_pert[0] - y_base[0]) * 1e-9, coords=[ds['time']], dims=['time'])
        ds['dco'] = xr.DataArray((y_pert[1] - y_base[1]) * 1e-9, coords=[ds['time']], dims=['time'])
        ds['doh'] = xr.DataArray((y_pert[2] - y_base[2]) * 1e-9, coords=[ds['time']], dims=['time'])
        ds['dh2'] = xr.DataArray((y_pert[3] - y_base[3]) * 1e-9, coords=[ds['time']], dims=['time'])
        ds = ds.fillna(0)
...
class AutoregressiveForecastingModel:
    ...
    """
    def __init__(self, working_directory, test_scenario, start_year, labels, features, diff=None):
    ...
    def scale_spatial(self, ds):
        ...
        self.s_scale_params = self.s_scale_params.assign_coords(variable=self.y + self.X)
        return ds
    ...
    def differentiate(self, ds):
        ...
        for var in self.diff:
            self.diff_params[var] = {f'{var}_init': ds[var].isel(time=0, scenario=0).values.tolist()}
    ...
    def integrate(self, ds):
        ...
        for var in self.diff:
            ...
            init_da = init_da.expand_dims(dim={'time': [ds['time'].values[0] - np.timedelta64(1, 'D')]})
    ...
    def array_to_dataset(self, arr):
        ...
        if len(arr.shape) == 5:
            return xr.Dataset(
                {var: (['scenario', 'time', 'lat', 'lon'], arr[:, :, :, :, i]) for i, var in enumerate(self.y + self.X)},
                coords={
            ...
            return xr.Dataset(
                {var: (['time', 'lat', 'lon'], arr[:, :, :, i]) for i, var in enumerate(self.y + self.X)},
                coords={
                    'time': self.dims['time'],
                    'lat': self.dims['lat'],
                    'lon': self.dims['lon']}
            ...
            return xr.Dataset(
                {var: (['scenario', 'time'], arr[:, :, i]) for i, var in enumerate(self.y + self.X)},
                coords={
                    'scenario': self.dims['scenario'],
                    'time': self.dims['time']
                }
    ...
    def autoregressor(self, model, arr_test):
        ...
        if len(arr_test.shape) == 3:
            ...
            for i in range(self.start, len(arr_test), self.n_step):
                ...
                pred = np.concatenate([pred_label, arr_test[i:i + self.n_step, 1:]], axis=-1)
                arr_roll = np.concatenate([arr_roll, pred], axis=0)[self.n_step:, :]
                arr_pred = np.concatenate([arr_pred, arr_roll[-self.n_step:, :]], axis=0)
        else:
            arr_test = arr_test[0, :, :, :, :]
            arr_roll = arr_test[self.start - self.length:self.start]
            arr_pred = arr_test[:self.start]
    ...
    def load_lstm(self, d, sp, max_runs=4, return_fn=False, get_fn=None):
        ...
        for f in range(max_runs):
            ...
            if return_fn:
                ...
                fn = get_fn[f]
            with open(self.wd + f'\\repository\\h2\\surrogate_models\\{sp}\\scale.json',
                      'r') as file:
    ...
    def crop_to_original_size(self, data, original_size, axis=3):
        ...
        if crop_size % 2 != 0:
            raise ValueError("The size difference is not divisible by 2. Padding or cropping might be misaligned.")
        ...
        slices[axis] = slice(crop_each_side, current_size - crop_each_side)  # Apply crop to the specified axis
    ...
    def load_o3(self, d, sp, max_runs=1, return_fn=False, get_fn=None):
        ...
        for f in range(max_runs):
            ...
            if return_fn:
                ...
                fn = get_fn[f]
            with open(glob.glob(self.wd + f'\\repository\\h2\\surrogate_models\\{sp}\\t_scale*')[0], 'r') as file:
                self.t_scale_params = json.load(file)
            with open(glob.glob(self.wd + f'\\repository\\h2\\surrogate_models\\{sp}\\s_scale*')[0], 'r') as file:
...
class RadiativeForcingModel:
    ...
    def compute_methane_radiative_forcing(self, ds):
        ...
        """
        return 1.14 * self.r_ch4 * (ds['ch4_trop'].sel(time=self.t) * 1e9 - ds['ch4_trop'].sel(time=self.t_0) * 1e9) * 1000
    ...
    def compute_tropospheric_ozone_radiative_forcing(self, ds):
        ...
        Returns:
            float: Radiative forcing (mW/m^2)
        """
        return self.r_o3 * (ds['o3_trop'].sel(time=self.t) * 1e9 - ds['o3_trop'].sel(time=self.t_0) * 1e9) * 1000
    ...
    def compute_stratospheric_water_vapour_radiative_forcing(self, ds):
        ...
        Returns:
            float: Radiative forcing (mW/m^2)
        """
        return self.r_h2o * (ds['h2o_strat'].sel(time=self.t) * 1e9 - ds['h2o_strat'].sel(time=self.t_0) * 1e9) * 1000


def run_case(wd, scenario, start_year=2035, t_mid=2060, m_adp=0.36, f_app=0.01, f_del=0.01, f_prod=0.01, kd=0.38):
    """
    ...
    doh_full_time = doh_pert.reindex_like(pert_dic[scenario]['oh_trop'], method='nearest', fill_value=0)
    ...
    relative_pert_oh = (doh_full_time / pert_dic_2d[scenario]['oh_trop'].where(pert_dic_2d[scenario]['oh_trop'] != 0)).fillna(0)
    pert_dic_2d[scenario]['oh_trop'] = pert_dic_2d[scenario]['oh_trop'] * (1 + relative_pert_oh)
    ...
    # 7a. CH4 Forecast
    pert_ch4_ds = msg.load_lstm(pert_dic, 'ch4_trop', max_runs=1, get_fn=fn_msg)[['ch4_trop']]
    ...
    ch4_forecast_aligned = ch4_forecast_avg.reindex_like(pert_dic_2d[scenario]['ch4_trop'], method='nearest')
    ...
    if 'lat' in pert_dic_2d[scenario]['ch4_trop'].dims:
        ch4_forecast_aligned = ch4_forecast_aligned.expand_dims({
            'lat': pert_dic_2d[scenario]['lat'],
            'lon': pert_dic_2d[scenario]['lon']
            }, axis=[1,2]) # Adjust axis numbers if necessary
    
    pert_dic_2d[scenario]['ch4_trop'] = ch4_forecast_aligned
    
    # 7c. O3 Forecast
    pert_o3_ds = osg.load_o3(pert_dic_2d, 'o3_trop', max_runs=1, get_fn=fn_osg)[['o3_trop']]
    ...
    o3_forecast_aligned = o3_forecast_avg.reindex_like(pert_dic[scenario]['o3_trop'], method='nearest')
    pert_dic[scenario]['o3_trop'] = o3_forecast_aligned
    # Also update the 1D CH4 value based on the perturbed forecast
    pert_dic[scenario]['ch4_trop'] = ch4_forecast_aligned.mean(dim=['lat', 'lon']) # Average the updated 2D CH4
    
    # 7e. Stratospheric H2O Forecast
    pert_swv_ds = wsg.load_lstm(pert_dic, 'h2o_strat', max_runs=1, get_fn=fn_wsg)[['h2o_strat']]
    
    # --- 8. Merge Final Forecast Results (Averaging runs) ---
    ds_base = xr.merge([base_ch4_ds, base_o3_ds.mean(dim=['lat', 'lon']), base_swv_ds]).mean(dim='run')
    ds_pert = xr.merge([pert_ch4_ds, pert_o3_ds.mean(dim=['lat', 'lon']), pert_swv_ds]).mean(dim='run')
    ...
    drf_ch4 = (
        rfm.compute_methane_radiative_forcing(ds_pert) -
        rfm.compute_methane_radiative_forcing(ds_base)
    )
    drf_o3 = (
        rfm.compute_tropospheric_ozone_radiative_forcing(ds_pert) -
    ...
    drf_h2o = (
        ...
        perturbation_ds['dh2'].sel(time=str(2100)+ '-01-01', method='nearest') * 1e9 * 0.03
    )
    ...
    result_ds['emih2'] = eh2_ds['emih2'].reindex_like(result_ds, method='nearest', fill_value=0)
    result_ds['h2'] = perturbation_ds['h2'].reindex_like(result_ds, method='nearest', fill_value=0) # Box model base H2
    result_ds['dh2'] = perturbation_ds['dh2'].reindex_like(result_ds, method='nearest', fill_value=0) # Box model H2 perturbation
    # Use the doh_full_time calculated earlier which is already aligned and zero-filled
    result_ds['doh'] = doh_full_time.reindex_like(result_ds, method='nearest') # Box model OH perturbation
    
    # Add OH concentration from perturbed simulation (before forecasting)
    result_ds['oh'] = pert_dic[scenario]['oh_trop'].reindex_like(result_ds, method='nearest')
...
if __name__ == '__main__':
    # Example use case
    working_directory = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # Fix ROOT_DIR issue
    scenario = 'ssp126'
