...
def calc_swv_rf(total_swv_mass: dict[str, np.ndarray]) -> dict[str, np.ndarray]:
    """Calculates RF due to a SWV perturbation mass.

    Based on Pletzer (2024) The climate impact of hypersonic transport.
...
def construct_myhre_1m_df(config: dict) -> pd.DataFrame:
    """Reproduce HALOE data from Myhre et al. (2007).

    This function produces the HALOE zonal mean vertical profile of
...
def get_volume_matrix(
    ...
    """Calculates the volume of every box of air in an altitude-latitude graph.

    The heights and latitudes arrays should have a spacing equivalent
...
def get_griddata(
    df: pd.DataFrame, heights: np.ndarray, latitudes: np.ndarray
) -> np.ndarray:
    """Transforms the data into an evenly spaced grid.

    Uses :func:`scipy.interpolate.griddata` to transform an input dataframe
    ``df`` into a :class:`numpy.ndarray`.

    Args:
        df (pandas.DataFrame): a dataframe containing altitudes and latitudes
            and corresponding values
        heights (numpy.ndarray): an of heights in meters
        latitudes (numpy.ndarray): an of latitudes in degrees
...
def get_alpha_aoa(
    ...
    """Calculate the fractional release factors and air-of-air.

    This function constructs the fractional release factors for CH4 (alpha)
    for different altitude and latitude levels. It then calculates a matrix
    of age-og-air values, rounded to whole years.

    Args:
        heights (numpy.ndarray): a np.array of heights in meters
        latitudes (numpy.ndarray): a np.array of latitudes in degrees
...
def calc_swv_mass_conc(
    ...
    and the Age-of-Air. Based on the thesis of A.J. Harmsen (2026): The Climate
    ...
    Args:
        delta_ch4 (numpy.ndarray): Array of yearly changes in CH4
    ...
    max_aoa = 6.0  # timelag map is defined up until 5.0
    if rounded_aoa.to_numpy().max() >= max_aoa:
        raise ValueError("rounded_aoa contains a value of 6 or higher.")
    if rounded_aoa.to_numpy().min() < 0.0:
...
def plot_swv_distribution(
    ...
    Args:
        latitudes (numpy.ndarray): A 1D array of latitude values in degrees
            north.
        heights (numpy.ndarray): A 1D array of the heights in meters.
        final_swv_distribution (pandas.DataFrame): A 2D frame of SWV
            distribution values.
...
def plot_alpha_aoa(
    ...
    Plot the alpha distribution as a latitude-pressure contour plot. Plot the
    rounded age-of-air distribution as a latitude-pressure heatmap.
    ...
    Args:
        latitudes (numpy.ndarray): A 1D array of latitude values in degrees
            north.
        heights (numpy.ndarray): A 1D array of the heights in meters.
        alpha (numpy.ndarray): A 2D array of alpha values.
        rounded_aoa (pandas.DataFrame): A 2D frame of age-of-air values rounded
