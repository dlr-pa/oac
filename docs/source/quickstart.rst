...
Example emission inventories,which were created as part of the DLR internal `Development Pathways for Aviation up to 2050 (DEPA 2050) <https://elib.dlr.de/142185/>`__ project, can be downloaded at `Zenodo <https://doi.org/10.5281/zenodo.11442323>`__.
These inventories comprise realistic emission data sets for global air traffic in 5-year steps between 2020 and 2050.
...
Depending on your use case, you may need to scale or normalise your emission inventories over time.
...
In OpenAirClim, this can be achieved using either a *scaling* or *normalisation*.
To understand the difference between normalisation and scaling, see :doc:`user_guide/evolution`.

Example normalisation and scaling files can be created using the following commands:
...
The script ``create_time_evolution.py`` creates two time evoluation files that control the temporal evoluation of the emission data, one for normalisation and the other for scaling.
...
To use either scaling or normalisation, simple uncomment one of the lines.
